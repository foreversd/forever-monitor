...
var Monitor = exports.Monitor = function (script, options) {
  ...
  this.command   = options.command || (
    os.platform() == 'win32' ? '"'+process.execPath+'"' : process.execPath
  );
...
Monitor.prototype.trySpawn = function () {
  ...
  if (os.platform() == 'win32') {
      var cmatch = this.command.match (re_safety_windows);
      console.log (this.command, cmatch);
      if (!cmatch) return false;
      command = cmatch[1] || cmatch[2];
      args = cmatch[3] ? 
          cmatch[3].split (' ').concat (this.args)
        : this.args
        ;
  } else {
    var cmatch = this.command.match (re_safety_linux);
    if (!cmatch) return false;
    command = cmatch[1];
    args = cmatch[2] ? 
        cmatch[2].split (' ').concat (this.args)
      : this.args
      ;
  }
