...
function watchTree (monitor, dir) {
...
exports.attach = function () {
  var monitor = this,
      watchDirectory = monitor.watchDirectory;

  fs.readFile(path.join(watchDirectory, '.foreverignore'), 'utf8', function (err, data) {
    if (err) {
    ...
    watchTree(monitor, watchDirectory);

    monitor.emit('watch:start');
  });
};
