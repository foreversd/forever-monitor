...
exports.attach = function(options) {
  ...
  function startLogs(_child, _childData) {
    if (monitor.child) {
      const stdoutEventName = options.stdoutEventName || 'stdout';
      monitor.child.stdout.on('data', function onStdout(data) {
        monitor.emit(stdoutEventName, data);
      });

      const stderrEventName = options.stderrEventName || 'stderr';
      monitor.child.stderr.on('data', function onStderr(data) {
        monitor.emit(stderrEventName, data);
      });
      ...
      if (options.customStdout) {
        monitor.child.stdout.pipe(options.customStdout, { end: false });
      }

      if (monitor.stderr) {
        monitor.child.stderr.pipe(monitor.stderr, { end: false });
      }
      if (options.customStderr) {
        monitor.child.stderr.pipe(options.customStderr, { end: false });
      }
    }
  }
};
